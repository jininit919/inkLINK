...
(function () {
  ...
  // Lišta se vykresluje dřív, než se stihne načíst i18n.js, a je to první
  // věc, kterou návštěvník vidí — proto si texty nese s sebou. Jazyk se
  // vybírá stejně jako tam: uložená volba, pak jazyk zařízení, pak angličtina.
  var STRINGS = {
    cs: {
      eyebrow: 'COOKIES',
      intro: 'Používáme nezbytné cookies pro přihlášení a analytické cookies pro zlepšování platformy. Marketing cookies <b>nepoužíváme</b>. Detail v ',
      policy: 'zásadách',
      acceptAll: 'PŘIJMOUT VŠE', essentialOnly: 'JEN NEZBYTNÉ', settings: 'NASTAVENÍ',
      settingsTitle: 'NASTAVENÍ COOKIES',
      settingsIntro: 'Nezbytné cookies jsou vždy aktivní — bez nich služba nefunguje. Ostatní si zapni podle uvážení.',
      essentialName: 'Nezbytné cookies',
      essentialDesc: 'Session pro přihlášení, CSRF, payment flow. Vyžadované zákonem k provozu služby.',
      analyticsName: 'Analytické cookies',
      analyticsDesc: 'Anonymní statistiky návštěvnosti, abychom viděli, co opravit / vylepšit. Bez profilování.',
      cancel: 'ZRUŠIT', save: 'ULOŽIT'
    },
    en: {
      eyebrow: 'COOKIES',
      intro: 'We use essential cookies for signing in and analytics cookies to improve the platform. We do <b>not</b> use marketing cookies. Details in our ',
      policy: 'privacy policy',
      acceptAll: 'ACCEPT ALL', essentialOnly: 'ESSENTIAL ONLY', settings: 'SETTINGS',
      settingsTitle: 'COOKIE SETTINGS',
      settingsIntro: 'Essential cookies are always on — the service does not work without them. The rest is up to you.',
      essentialName: 'Essential cookies',
      essentialDesc: 'Login session, CSRF, payment flow. Required by law to run the service.',
      analyticsName: 'Analytics cookies',
      analyticsDesc: 'Anonymous traffic statistics, so we can see what to fix or improve. No profiling.',
  ...
  var L = (function () {
    var saved = null;
    try { saved = localStorage.getItem('il_lang'); } catch (e) {}
    if (saved && STRINGS[saved]) return STRINGS[saved];
  ...
  function renderBanner() {
    ...
    el.innerHTML =
      ...
      '<div style="color:' + T.txt3 + ';font-size:10px;letter-spacing:0.18em;margin-bottom:6px">' + L.eyebrow + '</div>' +
      '<p style="margin:0 0 10px;color:' + T.txt2 + '">' + L.intro + '<a href="/privacy" style="color:' + T.accent + ';text-decoration:underline">' + L.policy + '</a>.</p>' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">' +
        '<button type="button" data-act="accept-all" style="background:' + T.btn + ';color:' + T.btnTxt + ';border:none;padding:9px 14px;font-family:inherit;font-size:11px;letter-spacing:0.1em;cursor:pointer">' + L.acceptAll + '</button>' +
        '<button type="button" data-act="essential" style="background:transparent;color:' + T.txt + ';border:1px solid ' + T.border + ';padding:9px 14px;font-family:inherit;font-size:11px;letter-spacing:0.1em;cursor:pointer">' + L.essentialOnly + '</button>' +
        '<button type="button" data-act="settings" style="background:transparent;color:' + T.txt2 + ';border:1px solid ' + T.border + ';padding:9px 14px;font-family:inherit;font-size:11px;letter-spacing:0.1em;cursor:pointer">' + L.settings + '</button>' +
      '</div>';
  ...
  function openSettings() {
    ...
    overlay.innerHTML =
      ...
      'max-width:480px;width:100%;padding:24px 26px;font-family:\'Helvetica Neue\',Helvetica,Arial,sans-serif">' +
        '<div style="color:' + T.txt3 + ';font-size:10px;letter-spacing:0.18em;margin-bottom:4px">' + L.settingsTitle + '</div>' +
        '<h2 style="margin:0 0 12px;font-size:18px;letter-spacing:0.06em;color:' + T.txt + '">Vyber kategorie</h2>' +
        '<p style="margin:0 0 18px;font-size:13px;color:' + T.txt2 + ';line-height:1.6">' + L.settingsIntro + '</p>' +

        '<div style="border:1px solid ' + T.border + ';padding:14px 16px;margin-bottom:10px">' +
          '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px">' +
            '<div><div style="font-size:13px;color:' + T.txt + ';margin-bottom:3px"><b>' + L.essentialName + '</b></div>' +
            '<div style="font-size:11px;color:' + T.txt3 + ';letter-spacing:0.04em">' + L.essentialDesc + '</div></div>' +
        ...
        '<div style="border:1px solid ' + T.border + ';padding:14px 16px;margin-bottom:18px">' +
          '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px">' +
            '<div><div style="font-size:13px;color:' + T.txt + ';margin-bottom:3px"><b>' + L.analyticsName + '</b></div>' +
            '<div style="font-size:11px;color:' + T.txt3 + ';letter-spacing:0.04em">' + L.analyticsDesc + '</div></div>' +
        ...
        '<div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap">' +
          '<button type="button" data-act="cancel" style="background:transparent;color:' + T.txt2 + ';border:1px solid ' + T.border + ';padding:9px 14px;font-family:inherit;font-size:11px;letter-spacing:0.1em;cursor:pointer">' + L.cancel + '</button>' +
          '<button type="button" data-act="save" style="background:' + T.btn + ';color:' + T.btnTxt + ';border:none;padding:9px 14px;font-family:inherit;font-size:11px;letter-spacing:0.1em;cursor:pointer">' + L.save + '</button>' +
        '</div>' +
      '</div>';
